...
#define UPDATE_KIDS_OFF(fsbuf, off, delta) do {\
uint32_t kids_off = get_kids_offset(fsbuf, off);\
if (kids_off) {\
...
#define COPY_TREE(kids_tree, kids_tree_size, tree_start_off, tree_end_off) do {\
...
static int do_remove_path(fs_buf* fsbuf, char* path, fs_change* changes, uint32_t* change_count, char** kids_tree, uint32_t* kids_tree_size)
{
	...
	if (name_off == DATA_START) {  // remove whole tree
...
__attribute__((visibility("default"))) void search_files(fs_buf* fsbuf, uint32_t *start_off, uint32_t end_off, const char* query, uint32_t* results, uint32_t* count)
{
	uint32_t size = *count;
	*count = 0;
	pthread_rwlock_rdlock(&fsbuf->lock);
