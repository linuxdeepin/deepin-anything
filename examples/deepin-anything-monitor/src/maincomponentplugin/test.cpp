...
struct nl_cb* prepare_sock(struct nl_sock* nl_sock)
{
    ...
    // 寻找广播地址
    int grp_id = genl_ctrl_resolve_grp(nl_sock, VFSMONITOR_FAMILY_NAME, VFSMONITOR_MCG_DENTRY_NAME);
    if (grp_id < 0)
    {
    ...
    // 设置广播接收
    nl_cb_set(nl_cb, NL_CB_VALID, NL_CB_CUSTOM, handle_msg_from_deepin_anything, NULL);
